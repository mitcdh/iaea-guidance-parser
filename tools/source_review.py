...
CHECKLIST = [
    ...
    "Account for every assigned discrepancy. Hidden objects, exclusions and ambiguous glyphs require Astra review; never guess or silently normalize them.",
        ...
        items.append(
            {
                ...
                "records": [compact_record(r) for r in selected],
...
def merge(folder, ledger, *, parsed=None, reviewer="Luna Max"):
    ...
    for item in packet["items"]:
        ...
        if packet["mode"] != "visual":
            raise ValueError(
                "Text results need Astra interpretation; they are not automated structural proof"
            )
        rows = [
