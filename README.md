...
PDF typography helps distinguish a footnote citation from an exponent or isotope
mass number. A raised digit after `cm` or `Pu` alone cannot establish a footnote
link. Genuine citations after units, such as `250 mSv` in GSG-11, can use an
explicit source-bound anchor after inspection. An uncertain association remains
an audit finding.

Scientific layout needs particular care. GSG-2's reviewed table pages preserve
native raised powers of ten as Unicode superscripts. Individually reviewed
equations can preserve a drawn fraction bar in explicit linear text, such as
`100 × (Value / Max(Value))` in NSS-24-G. These opt-in rules require the exact
source hash and retain review provenance. They do not repair other equations
automatically. Positional subscripts and isotope notation elsewhere may still
be flattened; consult the PDF when that loses meaning. Heading verification
preserves case and scientific characters: `cm²` must not pass as `cm2`, nor
`mSv` as `MSv`.

Recurring patterns belong in small parser rules; unusual layouts and damaged
fonts still need source-bound configuration. Judge a change by source-confirmed
corrections and current reviews. Fewer warnings or override entries alone do
...
Each completed publication gets a checkpoint containing its corrected records,
source evidence and review decisions. Promotion requires regeneration, export
checks and matching source, content and metadata fingerprints. Historical
checkpoints remain recoverable. The local baseline report
(`reviews/baseline-validation-results.md`) records coverage by method, resolved
defects, reproducibility checks and the unfinished queue. A fresh checkout
contains the reusable procedure, without those local verification claims.
