...
class IAEAGuidanceParser:
    ...
    def parse(self) -> tuple[DocumentMetadata, list[StructuralElement]]:
        ...
        active_footnote = _ActiveElement()

        for page in self.pages:
            # PDF text order normally places footnotes at the bottom of their
            # physical page. Close the prior page's footnote while allowing the
            # interrupted main paragraph to continue across the page boundary.
            self._flush_footnote(active_footnote)
            for line in self._prepared_page_lines(page):
                if not line or self._skip_line(line):
                    continue

                if CONTENTS_HEADING_RE.match(line) and not self.body_started and self.region == "FrontMatter":
                ...
                if active_table.kind == "table":
                    likely_post_table_heading = (
                        ...
                        and (len(active_table.lines) > 20 or bool(re.match(r"^STEP\s+\d+\s*:", line, re.I)))
                ...
                if active_footnote.kind == "footnote":
                    if footnote_match or self._line_starts_new_non_table_element(line):
                        self._flush_footnote(active_footnote)
                    else:
                        active_footnote.lines.append(line)
                        active_footnote.page_end_pdf = page.pdf_page
                        active_footnote.page_end_printed = page.printed_page
                        continue
    ...
    def _is_body_paragraph_id(self, para_id: str) -> bool:
        return bool(re.match(r"^(?:\d+(?:\.\d+)+|\d{3})$", para_id))
    ...
    def _is_body_start_after_contents(self, line: str, page: PageText) -> bool:
        ...
        if major:
            ...
            if major.group("num") == "1" and title.startswith("INTRODUCTION"):
                return True
        paragraph = self._paragraph_match(line)
        if page.printed_page != "1" or not paragraph:
            return False
        paragraph_id = canonical_dash(paragraph.group("id"))
