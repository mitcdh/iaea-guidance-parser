...
def _resolve_title(
    ...
    if _is_usable_title(inferred):
        if _prefer_filename_title(inferred, filename_title):
            return filename_title, "filename_replacing_incompatible_inference"
...
def _is_title_candidate(line: str) -> bool:
    ...
    if line in KNOWN_PUBLICATION_HEADINGS:
        return False
    normalized_label = re.sub(r"\s+", " ", line).strip().lower()
    non_title_cover_labels = {
        *(label.lower() for label in DOCUMENT_CATEGORY_LABELS),
        "reference manual",
        "training guide",
        "service guide",
    }
    if normalized_label in non_title_cover_labels:
        return False
...
def _prefer_filename_title(inferred: str, filename_title: str) -> bool:
    """Use a descriptive filename when an extracted cover title is incompatible.

    A few older PDFs have broken character maps that turn a short cover label
    into plausible-looking alphabetic gibberish; others omit a short title
    fragment during extraction. The checks deliberately require a descriptive,
    compatible filename, so generic names such as ``PUB123_web.pdf`` do not
    override usable PDF metadata.
    """
    if not _is_usable_title(filename_title):
        return False
    inferred_token_list = re.findall(r"[a-z0-9]+", inferred.lower())
    filename_token_list = re.findall(r"[a-z0-9]+", filename_title.lower())
    inferred_tokens = set(inferred_token_list)
    filename_tokens = set(filename_token_list)
    incompatible_short_title = (
        len(inferred_tokens) <= 3
        and len(filename_tokens) >= 5
        and not inferred_tokens.intersection(filename_tokens)
    )
    truncated_cover_title = (
        len(filename_token_list) > len(inferred_token_list)
        and filename_token_list[: len(inferred_token_list)] == inferred_token_list
        and len(filename_token_list) - len(inferred_token_list) <= 3
    )
    expanded_filename_title = (
        len(inferred_tokens) >= 4
        and inferred_tokens < filename_tokens
        and len(filename_tokens) - len(inferred_tokens) <= 6
    )
    return incompatible_short_title or truncated_cover_title or expanded_filename_title
