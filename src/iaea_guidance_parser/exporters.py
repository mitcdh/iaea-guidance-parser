...
def write_outputs(
    out_dir: Path, metadata: DocumentMetadata, records: list[StructuralElement]
) -> None:
    ...
    findings = run_records_qa(
        records,
        manifest_doc_ids={metadata.document_id},
        manifest_counts={metadata.document_id: len(records)},
    )
    write_qa_json(
...
def write_series_outputs(
    ...
    *,
    ...
    write_jsonl(
        out_dir / "series_custom_gpt_knowledge.jsonl",
        [to_custom_gpt_record(r) for r in all_records],
    )
    write_series_markdown_knowledge(
        out_dir / "series_custom_gpt_knowledge.md", series_config or {}, results, run_info=run_info
    )
    ...
    write_series_qa_report(
        out_dir / "series_qa_report.md",
        series_config or {},
        results,
        failures,
        findings=findings,
    ...
    write_qa_markdown(
        out_dir / "qa_report.md", findings, title="Series QA report", metadata=run_info
    )
...
def build_series_run_info(series_config: dict[str, Any], results: list[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "document_manifest_checksum": fingerprint(documents),
...
def write_markdown_knowledge(
    path: Path, metadata: DocumentMetadata, records: list[StructuralElement]
) -> None:
    with path.open("w", encoding="utf-8") as f:
...
def write_qa_report(
    ...
    findings = run_records_qa(records)
    write_qa_markdown(
        path,
        findings,
        metadata={
            ...
            "element_counts": dict(Counter(r.element_type for r in records)),
        },
    )
    ...
    *,
    ...
    for index, part in enumerate(parts, start=1):
        ...
        with part_path.open("w", encoding="utf-8") as f:
            f.write(
                _series_part_preamble(
                    series_config, index, total_parts, part["documents"], run_info=run_info
                )
            )
...
def format_record_label(record: StructuralElement) -> str:
    return (
        f"{record.element_type} {record.element_id}" if record.element_id else record.element_type
    )
...
def _record_content_text(record: StructuralElement) -> str:
    ...
    if record.extra.get("reviewed_equations"):
        extras.append("Source note: equation layout rendered as reviewed linear text.")
...
def _table_markdown(record: StructuralElement) -> str:
    """HTML cells preserve empty cells and spans that Markdown pipes cannot."""
    table = record.extra["table"]
    rows = [record.title or record.element_id or "Table", "", "<table>"]
    for row in range(table["row_count"]):
        rows.append("<tr>")
        for cell in sorted(
            (c for c in table["cells"] if c["row"] == row), key=lambda c: c["column"]
        ):
            attributes = ""
            if cell["row_span"] > 1:
...
def write_series_manifest(
    ...
    *,
    ...
    for failure in failures:
        docs.append(
            {"status": "failed", "source_pdf": str(failure.source_pdf), "error": failure.error}
        )
    ...
    write_json(
        path,
        {
            ...
            "runtime": (run_info or {}).get("runtime", {}),
...
def write_series_qa_report(
    ...
    *,
    ...
    """Use the same finding format for single documents and complete series."""
    ...
    metadata = {
        **(run_info or {}),
        ...
        "record_count": len(records),
        "element_counts": dict(Counter(r.element_type for r in records)),
        "status_counts": dict(Counter(r.text_status for r in records)),
    }
    write_qa_markdown(path, findings or [], title="Series QA report", metadata=metadata)
