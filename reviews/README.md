...
When present, local `WORK.md` identifies the single open-case queue, ownership rules and active correction batch. `baseline-validation-results.md` records the latest baseline acceptance, coverage by method, source-confirmed corrections and remaining work. `parser-integration-results.md` is a historical checkpoint, not a statement of current coverage.
...
Use the bounded preparation and merge helper described in [Source audit and review](../docs/auditing.md). Workers write only assignment results; the review coordinator samples passes and records adjudications before merging. A failed sample reopens the assignment, and stale or altered evidence is rejected. Do not copy a new fingerprint into an old review to make it current.
...
Baseline acceptance covers explicitly recorded publications and pages. It does
not certify the remaining corpus. Preserve pending and blocked cases when
rebuilding the queue, including prior blocked reviews whose fingerprints became
stale. Their earlier evidence remains useful, but requires fresh acceptance.
