...
- `reviewed_equations` linearizes individually inspected equations when layout
  carries meaning absent from extracted text. It requires a source SHA-256,
  physical page, paragraph label, exact `original_text`, `linear_text`, reviewer
  and source notes. An absent or ambiguous match fails parsing. The record keeps
  the original extraction and review provenance; reading exports identify the
  linearization. NSS-24-G equation (2) preserves a drawn fraction bar; GSG-2
  equation (6) preserves a sum of concentration ratios and its strict inequality.
  This does not infer or normalize other mathematical expressions;
- `reviewed_superscripts` preserves native PDF spans marked as raised on
  reviewed pages, and reads those pages in geometric order. Supply a source
  SHA-256, `pages`, `reviewer` and `notes`. Supported digits, signs and note
  letters become Unicode superscripts; unsupported raised characters fail
  extraction. Ordinary digits and subscripts are unchanged. GSG-2 uses this
  for powers of ten, isotope mass numbers and notes in TABLE 10 on pages
  59–64. Original span text and geometry remain available during extraction;
