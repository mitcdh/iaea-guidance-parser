...
Screening normalization does not certify scientific notation. Heading proofs
use case-sensitive wording and preserve superscript/subscript characters
(`pdf_bookmark_tree_v2`). Exponents, isotopes and units also need their original
characters and positional evidence where extraction flattens their meaning.
...
The audit's `integrity_failures` summary also counts unresolved footnote and
layout associations. A nonzero audit exit therefore requires reading the
finding types. Report serialization, manifest, source-hash and export failures
separately from source-review candidates; never describe a completed audit as
complete source verification. The local baseline report records both outcomes.
...
Use `--geometry` when ownership, placement or notation needs source spans. The
packet includes geometry only for assigned and context pages. Merge validates
the current parsed fingerprints of context pages as well as owned pages.

## Delegate comparisons, retain judgment

Use fresh comparison workers with only their assigned evidence and no more than three concurrent workers. The review coordinator retains planning, ambiguity resolution, corrections and exclusion-rule approval. Supply only the packet path and this bounded instruction:
...
Compare the rendered source with the packet's top-level `records`. The
`items[].source_text` field is independent extraction evidence and can contain
damaged font codes. Its disagreement with the image is not itself a parser
defect; cite the actual parsed record when reporting one.

For text-only investigations, the review coordinator interprets the result separately. A worker's text comparison cannot be merged as automated structural proof.
...
Freeze the current code, dependency versions, effective configurations, source
hashes, records and review evidence before making corrections. Historical counts
are checkpoints to reconcile. Keep this run inventory and reports in ignored
local locations, with the reusable procedure in Git.

Check affected publications first. Compare meaningful content, hierarchy,
pagination, geometry and associations against both the previous generation and
the source. Matching previous output detects a regression; it cannot prove that
the previous output was accurate. Include scientific notation, footnote owners,
glossary definitions, captions, tables and technical boundaries explicitly.

Once code stabilizes, regenerate both series from a frozen runtime. Configuration
corrections found during the comparison require affected-document reparsing and
complete combined re-export. Bind the final effective configurations for every
document and rerun independent audits. Reject unexpected changes before
promotion. Reproduce representative corrected publications through the normal
CLI and compare their per-document exports byte for byte. Combined run IDs and
output paths describe each execution and are not scientific content.

Promote reviews only when their current source, content and metadata fingerprints
match. Preserve earlier generations and evidence; a relocated historical export
can be recovered by its exact bytes without rewriting the original manifest or
review. Create a fresh queue from the new audits, retaining unresolved findings
and separate metadata/boundary checks. Report verified closures, confirmed
defects and review effort; packet bytes and image counts are cost proxies when
