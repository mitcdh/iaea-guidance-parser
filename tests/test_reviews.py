...
@pytest.mark.parametrize("source,changed", [("cm²", "cm2"), ("mSv", "MSv")])
def test_heading_proof_preserves_scientific_notation(source, changed):
    import fitz
    ...
    with fitz.open() as doc:
        page = doc.new_page()
        title = "DOSE IN " + source
        page.insert_text((50, 80), title)
        ...
        record = {
            ...
            "section_path": [title],
            "source_region": "Body",
            "extra": {"source_spans": [{"pdf_page": 1, "bbox": list(page.search_for(title)[0])}]},
        }
        assert structural_proof(doc, 1, [record], title)
        record.update(text="DOSE IN " + changed, section_path=["DOSE IN " + changed])
        assert structural_proof(doc, 1, [record], title) is None
