...
# These assertions audit a specific, previously generated 16,338-record output
# snapshot that is intentionally gitignored. Keep the normal unit suite
# runnable from a clean checkout; opt in only when that snapshot is present.
pytestmark = pytest.mark.skipif(
    os.environ.get("IAEA_RUN_SECURITY_AUDIT_SNAPSHOT") != "1",
    reason="requires the archived outputs/Security snapshot; set IAEA_RUN_SECURITY_AUDIT_SNAPSHOT=1 to run",
)
